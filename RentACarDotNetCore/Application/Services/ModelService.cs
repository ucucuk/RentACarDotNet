...
namespace RentACarDotNetCore.Application.Services
{
	public class ModelService : IModelService
	{
		...
		{
			Brand brand = _brands.Find(brand => brand.Name.ToUpper().Equals(updateModelRequest.Brand.Name.ToUpper())).FirstOrDefault();
			if (brand == null)
			{
			...
			var nameFilter = Builders<Model>.Filter.Eq(model => model.Name, updateModelRequest.Name.ToUpper());
			...
			if (isDuplicated != null)
				throw new AlreadyExistsException($"{updateModelRequest.Name.ToUpper()} model already exists.");
			...
			model.Name = updateModelRequest.Name.ToUpper();
			model.Brand = brand;
